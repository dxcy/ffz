...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/common.js"></script>
 ...
 <script type="text/javascript" charset="UTF-8">
                            var AtlasType_datagrid;
                            var i = 0;
                            var AtlasTypeDialog;
                            var EditeAtlasTypeForm;
                            FMG.editeRow = undefined; //记录正在编辑的行号
                            $(function() {

                                AtlasType_datagrid = $("#AtlasType_datagrid").datagrid({
                                    url: '${pageContext.request.contextPath}/AtlasType/getAllAtlasTypeByFenye.do',
                                    title: '',
                                    iconCls: 'icon-save',
                                    cache: false,
                                    loadMsg: "查找中，请等待 …",
                                  ...
                                  columns: [[{
                                    ...
                                    toolbar: [{
                                        ...
                                        handler: function() {
                                            if (FMG.editeRow != undefined) { //只能开启一行编辑状态
                                                AtlasType_datagrid.datagrid('endEdit', FMG.editeRow);
                                            }
                                            if (FMG.editeRow == undefined) {

                                                $('#EditeAtlasTypeForm').form('clear');
                                                
                                                AtlasTypeDialog = $('#AtlasTypeDialog').dialog({
                                                    title: "添加",
                                                    modal: true,
                                                    buttons: [{
                                                        text: '添加图集类型',
                                                        handler: function() {
                                                            ...
                                                            if ($('#EditeAtlasTypeForm').form('validate')) {
                                                                FMG.editeRow = 0;
                                                                $.ajax({
                                                                    type: "POST",
                                                                    url: '${pageContext.request.contextPath}/AtlasType/addAtlasType.do',
                                                                    data: $('#EditeAtlasTypeForm').serialize(),
                                                                    cache: false,
                                                                    dataType: 'json',
                                                                    success: function(msg) {
                                                                        if (msg && msg.success == true) {
                                                                            AtlasTypeDialog.dialog('close');
                                                                            $.messager.show({
                                                                                title: '提示',
                                                                                msg: '添加' + msg.result.name + '成功'
                                                                            });
                 
                                                                            AtlasType_datagrid.datagrid('load', FMG.serializeObject($('#AtlasType_searchForm').form()));

                                                                        } else {
                                                                            $.messager.alert('标题', msg.result);
                                                                            AtlasType_datagrid.datagrid('rejectChanges');
                                            //取消被选择的号
                                            AtlasType_datagrid.datagrid('unselectAll');
                                                                        }
                                    ...
                                    '-', {
                                        ...
                                        iconCls: 'icon-remove',
                                        handler: function() {
                                            var rows = AtlasType_datagrid.datagrid('getSelections');
                                            ...
                                            if (rows.length!=null && rows.length > 0) {
                                                ...
                                                function(b) { //当用户点击确定，b=true
                                                    if (b) {
                                                        ...
                                                        $.ajax({
                                                            type: "POST",
                                                            url: '${pageContext.request.contextPath}/AtlasType/delAtlasType.do?id=' + ids.join(','),
                                                            cache: false,
                                                            dataType: 'json',
                                                            success: function(msg) {
                                                                if (msg && msg.success == true) {
                                    ...
                                    '-', {
                                        ...
                                        handler: function() {
                                            ...
                                            if (rows.length > 1) {
                                                ...
                                                $('#EditeAtlasTypeForm').find('[name = id]').attr('readonly', 'readonly');
                                                $('#EditeAtlasTypeForm').form('clear');

                                                $('#EditeAtlasTypeForm').form('load', {
                                                    id: rows[0].id,
                                                    name: rows[0].name,
                                                    ...
                                                    buttons: [{
                                                        ...
                                                        handler: function() {
                                                            if ($('#EditeAtlasTypeForm').form('validate')) {
                                                                $.ajax({
                                                                    type: "POST",
                                                                    url: '${pageContext.request.contextPath}/AtlasType/updateAtlasType.do?id=' + rows[0].id,
                                                                    data: $('#EditeAtlasTypeForm').serialize(),
                                                                    cache: false,
                                                                    dataType: 'json',
                                                                    success: function(msg) {
                                                                        if (msg && msg.success == true) {
                                                                            ...
                                                                            AtlasType_datagrid.datagrid('reload');
                                                                      AtlasType_datagrid.datagrid('unselectAll');
                                                                      AtlasType_datagrid.datagrid('clearSelections');
                                                                        } else {
                                    ...
                                    onAfterEdit: function(rowIndex, rowData, changes) { //编辑行索引，编辑行数据，原先数据和新数据的键值对
                                        ...
                                        var url = '${pageContext.request.contextPath}/AtlasType/updateAtlasType.do?id=' + rowData.id;
                                            $.ajax({
                                                type: "POST",
                                                url: url,
                                                data: rowData,
                              ...
                              /*      onDblClickRow: function(rowIndex, rowData) {
                                        ...
                                        if (FMG.editeRow == undefined) {
                                            AtlasType_datagrid.datagrid('beginEdit', rowIndex);
                                ...
                                searchAtlasTypesByConditions = function() {
                                    AtlasType_datagrid.datagrid('load', FMG.serializeObject($('#AtlasType_searchForm').form()));
                                   

                                };

                                clearSearchAtlasType = function() {
                                    FMG.clearForm($('#AtlasType_searchForm').form());
                                    AtlasType_datagrid.datagrid('load', {});
                                };
...
<form id="AtlasType_searchForm" action="" method="post">
...
class="easyui-linkbutton" onclick="searchAtlasTypesByConditions()">
	查询 </a> <a href="javascript:void(0);" class="easyui-linkbutton"
onclick="clearSearchAtlasType()"> 清空 </a></td>
			</tr>
		</table>
	</form>
</div><!-- 查询 -->
...
<div title="图集类型管理" region="center" border="false">
	<table id="AtlasType_datagrid">
	</table>
</div>
<div id="p" style="width:400px;"></div>

</div><!-- layout -->
<div id="AtlasTypeDialog"
	style="width: 300px;height: 200px;text-align: center;
	">
<form id="EditeAtlasTypeForm" action="" method="post">
