...
public class ydDataController {
	...
	@RequestMapping(value = "/catalogSynchro")
	public void catalogSynchro(HttpServletRequest request,
		...
		util util = new util();
		try {
			cataUtil.catalogSynchro( ydDataInter, dataTypeInter,ydUserInter, userTypeServiceInter);
			flag = true;
			util.writeJson("同步完成", flag, request, response);
